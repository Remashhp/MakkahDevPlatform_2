...
$sql .= " ORDER BY course_title $order";
...
<?php endif; ?>
  ...
  <main style="margin-top: 120px;"> 
    <div class="container-fluid px-4 px-lg-5">
      ...
      <div class="row">
        <div class="col-12">
          <h2 class="page-title text-end mb-3 fs-1 fs-md-1">البرامج التدريبية</h2>
    ...
    <form method="GET" id="filterForm">
      <div class="row mb-4 g-3" style="margin-top: 40px; margin-right: 40px;">
        <!-- Search -->
        <div class="col-12 col-md-6 col-lg-3">
          <div class="icon-input">
            <i class="fas fa-search"></i>
            <input type="text" class="form-control filter-input" placeholder="البحث">
          </div>
        </div>
        ...
        <div class="col-12 col-md-6 col-lg-3 ">
          <div class="icon-input">
            ...
            <input type="text" id="date-input" class="form-control filter-date" placeholder="اختر التاريخ">
          </div>
        </div>
        <!-- Course Category & Alphabetical Sort -->
        <div class="col-12 col-md-6 col-lg-3 d-flex" style="gap:15px;">
          <div class="custom-select" style="width: 50%;">
            <select class="form-select">
              <option>الكل</option>
              <option>مشترك</option>
              <option>تخصصية</option>
            </select>
            <i class="fas fa-chevron-down arrow-icon"></i>
          </div>
          <div class="custom-select" style="width: 50%;">
            <select class="form-select">
              <option>من أ → ي</option>
              <option>من ي → أ</option>
            </select>
            <i class="fas fa-chevron-down arrow-icon"></i>
          </div>
        </div>
...
$query = "SELECT * FROM course_info ORDER BY start_date ASC";
...
if ($result->num_rows === 0) {
    echo "<p>لا توجد دورات متاحة حاليا.</p>";
} else {
    echo '<div class="row g-2 justify-content-start">';
    while ($course = $result->fetch_assoc()) {
        // تحديد لون الحالة
        $statusColor = $course['status'] === 'متاح للتسجيل' ? '#1BCC4A' : '#F00';
        ?>
        <div class="col-12 col-sm-6 col-lg-3 d-flex justify-content-center">
            <a href="Course_Details.php?id=<?= (int)$course['id'] ?>" style="text-decoration: none; color: inherit;">
                <div class="program-card" style="width: 270px;">
                    <img src="<?= htmlspecialchars($course['course_image']) ?>" alt="<?= htmlspecialchars($course['course_title']) ?>" class="img-fluid">
                    <div class="p-3">
                        <p class="status mb-2 mt-2" style="color: <?= $statusColor ?>;">
                            <i class="fa-solid fa-circle" style="color: <?= $statusColor ?>; font-size: 10px;"></i>
                            <?= htmlspecialchars($course['status']) ?>
                        </p>
                        <h3 class="program-title mb-3"><?= htmlspecialchars($course['course_title']) ?></h3>
                        <div class="program-info d-flex gap-3">
                            <p class="mb-0">
  ...
  <script>
    ...
    flatpickr("#date-input", {
      ...
      onReady: function(selectedDates, dateStr, instance) {
        instance.input.setAttribute("placeholder", "اختر التاريخ");
      }
    });
  </script>
